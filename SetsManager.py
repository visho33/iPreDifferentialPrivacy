...
def sparse_data(size, Z, Z_test):
    newZ = zeros((len(Z), size))
    newZ_test = zeros((len(Z_test), size))
    selected = choice([i for i in range(1, size)], len(Z[0]) - 1, replace = False)
    for i in range(len(Z)):
        newZ[i][0] = 1
        for j in range(len(selected)):
            newZ[i][selected[j]] = Z[i][j+1]
    
    for i in range(len(Z_test)):
        newZ_test[i][0] = 1
        for j in range(len(selected)):
             newZ_test[i][selected[j]] = Z_test[i][j+1]

    return newZ, newZ_test
